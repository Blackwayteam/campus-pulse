...
export default function CampusPage() {
  ...
  const [newAlert, setNewAlert] = useState<FeedItem | null>(null)
  ...
  function subscribeToUpdates() {
    ...
    channelRef.current = supabase
      ...
      .on('postgres_changes', {
        ...
        // Only play sound if someone else posted it
        if (item.created_by !== user?.id) {
          playSound(item.status)
        }
        setNewAlert(item)
      ...
      // REACTIONS — everyone hears everyone's reactions
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'reactions',
        ...
        // Play sound for OTHER people's reactions
  ...
  // Spawn emoji particles at click position
  ...
  // Unlimited reactions — each click adds one
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      {/* PARTICLE OVERLAY — fixed, covers full screen */}
      ...
      <AnimatePresence>
        {newAlert && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -60 }}
            className={`mx-4 mt-3 rounded-2xl p-4 border-2 ${
              newAlert.status === 'cancelled' ? 'bg-red-500/20 border-red-500' :
              newAlert.status === 'confirmed' ? 'bg-blue-500/20 border-blue-500' :
              newAlert.status === 'pending'   ? 'bg-orange-500/20 border-orange-500' :
          ...
          >
            <div className="flex items-center gap-3">
              ...
              >
                {STATUS_EMOJI[newAlert.status] ?? '📣'}
              </motion.span>
              <div className="flex-1">
                <p className="text-white font-black text-sm">
                  {newAlert.status === 'cancelled' && '🔥 CLASS IS BURNING'}
                  {newAlert.status === 'confirmed' && '✅ CLASS IS ON — MOVE'}
                  {newAlert.status === 'pending'   && '⚠️ STAY TUNED'}
                  {newAlert.status === 'delayed'   && '⏱️ CLASS DELAYED'}
                  {newAlert.status === 'broadcast' && '📣 BROADCAST'}
                </p>
                {newAlert.message && (
              ...
              {feed.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* REACTIONS — unlimited, sound + particles on every click */}
...
// Building block with fire/rain animations
